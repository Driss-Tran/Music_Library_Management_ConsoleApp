...
public class TrackDAO implements TrackDAOInterface<Track>{
	...
	public int deleteTrack(int id,Track value) {
		...
		try {
			...
			String delsql = "DELETE FROM PLAYLISTS WHERE TRACK_ID = (select idTracks from Tracks where title = ?)";
			PreparedStatement prepare = conn.prepareStatement(sql);
			PreparedStatement prepare1 = conn.prepareStatement(delsql);
			prepare.setInt(1,id);
			prepare1.setString(1,value.getTitle());
			rs = prepare1.executeUpdate();
			rs = prepare.executeUpdate();
			}
		catch (SQLException e) {
			// TODO Auto-generated catch block
