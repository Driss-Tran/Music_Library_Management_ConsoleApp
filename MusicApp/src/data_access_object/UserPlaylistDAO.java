...
public class UserPlaylistDAO implements UserPlaylistDAOInterface<UserPlaylist>{
	...
	public int deletePlaylist( UserPlaylist value) {
		...
		try {
			...
			String delsql1 = "DELETE FROM PLAYLISTS WHERE USERPL_ID = (select id from userplaylists where name_playlist = ?) AND USER_ID = ?";
			...
			if (rs.next()) {
				...
				delprep1.setString(1,value.getName_playlist());
				delprep1.setInt(2,value.getUser_id());
				delprep1.executeUpdate();
